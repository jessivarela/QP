...
det10 <- det10 %>%
  ...
  dplyr::select(participant, target, bin, noun_transparency, structure, gender,
                target_count, target_prop, eLog, wts, onset_target) %>%   ### change onset_target in previous line depending on what trigger we want to observe
...
write_csv(det10, here::here("data", "clean", "det_10ms_onset_target.csv"))


# -----------------------------------------------------------------------------

det10 <- read_csv(here::here("data", "clean", "det_10ms_onset_target.csv"))

det10plot <- det10

det10plot$noun_transparency <- factor(det10plot$noun_transparency, levels = c("opaq", "transp"), 
                        labels = c("Opaque nouns", "Transparent nouns"))

det10plot$group <- factor(det10plot$group, levels = c("hs", "l2", 'ss'), 
                          labels = c("HS", "L2", 'SS'))

timecourse10_type <- det10plot %>%
  filter(time_zero > -60, time_zero < 40) %>% 
  ggplot(., aes(x = time_zero, y = target_prop, color = group)) +
  facet_grid(. ~ noun_transparency) +
  geom_vline(xintercept = 20, lty = 3) +
  geom_hline(yintercept = 0.5, color = "grey", size = 3) +
  stat_summary(fun.y = mean, geom = "line") +
  ggtitle("Time course type of noun") +
  xlab("Time in 10 ms bins (0 = adjective onset)") +
...
timecourse10_group <- det10plot %>%
  filter(time_zero > -60, time_zero < 40) %>% 
  ggplot(., aes(x = time_zero, y = target_prop, color = noun_transparency)) +
  facet_grid(. ~ group) +
  geom_vline(xintercept = 20, lty = 3) +
  geom_hline(yintercept = 0.5, color = "grey", size = 3) +
  stat_summary(fun.y = mean, geom = "line") +
...
ggsave('timecourse10_nountype.png',
       plot = timecourse10_type, dpi = 600, device = "png",
       path = './figs/timecourse/',
       height = 3.5, width = 8.5, units = 'in')

ggsave('timecourse10_group.png',
       plot = timecourse10_type, dpi = 600, device = "png",
       path = './figs/timecourse/',
       height = 3.5, width = 8.5, units = 'in')
